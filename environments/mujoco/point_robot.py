...
class SparsePointEnv(PointEnv):
    '''
     ...
     '''
    def __init__(self, randomize_tasks=True, n_tasks=100, goal_radius=0.3, max_episode_steps=200):
    ...
    def step(self, action):
        ...
        d.update({'sparse_reward': sparse_reward,'task':self._goal})
...
class SparsePointSubEnv(PointEnv):
    '''
     - tasks sampled from unit half-circle
     - reward is L2 distance given only within goal radius
     ...
     the algorithm should call `sparsify_rewards()` to get the sparse rewards
     '''
    def __init__(self, randomize_tasks=True, n_tasks=2, goal_radius=0.3, max_episode_steps=200):
        self._max_episode_steps = max_episode_steps
        super().__init__(randomize_tasks, n_tasks)
        self.goal_radius = goal_radius
