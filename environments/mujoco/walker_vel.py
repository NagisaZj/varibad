...
class WalkerSparseEnv(WalkerVelEnv):
    """Half-cheetah environment with target velocity, as described in [1]. The
    code is adapted from
    https://github.com/cbfinn/maml_rl/blob/9c8e2ebd741cb0c7b8bf2d040c4caeeb8e06cc95/rllab/envs/mujoco/half_cheetah_env_rand.py
    ...
    [2] Emanuel Todorov, Tom Erez, Yuval Tassa, "MuJoCo: A physics engine for
        ...
        (https://homes.cs.washington.edu/~todorov/papers/TodorovIROS12.pdf)
    """
    def __init__(self, task={}, n_tasks=2, randomize_tasks=True,goal_radius=0.5, max_episode_steps=200):
        self._max_episode_steps = max_episode_steps
        self._task = task
        self.tasks = self.sample_tasks(n_tasks)
        self._goal_vel = self.tasks[0]
        self._goal = self._goal_vel
        self.goal_radius= goal_radius
        super(WalkerSparseEnv, self).__init__()
    ...
    def step(self, action):
        ...
        infos = dict(reward_forward=forward_reward,
            reward_ctrl=-ctrl_cost, velocity=forward_vel,sparse_reward=sparse_reward,task=self._goal_vel)
        return observation, reward, done, infos
